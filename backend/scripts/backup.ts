...
function sanitizePath(userPath: string, baseDir: string): string {
  ...
  if (
    !resolvedPath.startsWith(resolvedBaseDir + sep) &&
    resolvedPath !== resolvedBaseDir
  ) {
    throw new Error(
      'Path traversal detected: path must be within allowed directory'
    );
  }
...
function validateConfig(config: BackupConfig): void {
  ...
  if (
    resolvedBackupDir !== DEFAULT_BACKUP_DIR &&
    !resolvedBackupDir.startsWith(DEFAULT_BACKUP_DIR + sep)
  ) {
    throw new Error(
      `Invalid backup directory: ${config.backupDir} is not within allowed directories`
    );
  }
}
    ...
    try {
      switch (command) {
        ...
        case 'restore':
          if (!backupPath) {
            console.error(
              'Backup path required: npm run backup:restore <backup-path>'
            );
            process.exit(1);
          }
        ...
        default:
          console.log(
            'Usage: ts-node backup.ts [create|restore <path>|list|cleanup|scheduled]'
          );
          process.exit(1);
      }
