...
describe('Metrics API', () => {
  ...
  describe('Metrics tracking functions', () => {
    it('should track user registrations', async () => {
      ...
      const registrationMetric = metricsData.find(
        m => m.name === 'user_registrations_total'
      );
    ...
    it('should track error submissions', async () => {
      ...
      const metricsData = await metrics.getMetrics();
      const errorMetric = metricsData.find(
        m => m.name === 'error_submissions_total'
      );
    ...
    it('should track solution submissions', async () => {
      ...
      const metricsData = await metrics.getMetrics();
      const solutionMetric = metricsData.find(
        m => m.name === 'solutions_submitted_total'
      );
    ...
    it('should track solution acceptances', async () => {
      ...
      const metricsData = await metrics.getMetrics();
      const acceptanceMetric = metricsData.find(
        m => m.name === 'solutions_accepted_total'
      );
    ...
    it('should track database queries', async () => {
      ...
      const metricsData = await metrics.getMetrics();
      const queryMetric = metricsData.find(
        m => m.name === 'database_queries_total'
      );
      const durationMetric = metricsData.find(
    ...
    it('should track database connection errors', async () => {
      ...
      const metricsData = await metrics.getMetrics();
      const errorMetric = metricsData.find(
        m => m.name === 'database_connection_errors_total'
      );
      expect(errorMetric).toBeDefined();
    });
  ...
  describe('Metrics middleware', () => {
    ...
    it('should track different HTTP status codes', async () => {
      ...
      expect(response.text).toContain(
        'http_requests_total{method="GET",route="/nonexistent-route",status="404"}'
      );
    });
  });
});
