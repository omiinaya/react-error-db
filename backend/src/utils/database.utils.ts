...
export const recordExists = async (
  ...
  try {
    const modelMap = {
      ...
      application: () =>
        prisma.application.findUnique({ where: { id }, select: { id: true } }),
      errorCode: () =>
        prisma.errorCode.findUnique({ where: { id }, select: { id: true } }),
      solution: () =>
        prisma.solution.findUnique({ where: { id }, select: { id: true } }),
      vote: () =>
        prisma.vote.findUnique({ where: { id }, select: { id: true } }),
      userSession: () =>
        prisma.userSession.findUnique({ where: { id }, select: { id: true } }),
      auditLog: () =>
        prisma.auditLog.findUnique({ where: { id }, select: { id: true } }),
      categoryRequest: () =>
...
export const getPaginatedResults = async <T>(
  ...
  const modelMap = {
    ...
    application: () => ({
      findMany: () =>
        prisma.application.findMany({ ...queryOptions, skip, take }),
      count: () => prisma.application.count({ where: queryOptions.where }),
    }),
    errorCode: () => ({
      findMany: () =>
        prisma.errorCode.findMany({ ...queryOptions, skip, take }),
    ...
    userSession: () => ({
      findMany: () =>
        prisma.userSession.findMany({ ...queryOptions, skip, take }),
    ...
    categoryRequest: () => ({
      findMany: () =>
        prisma.categoryRequest.findMany({ ...queryOptions, skip, take }),
