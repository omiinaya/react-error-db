...
async function testThemePreferenceFetching() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `   Error Message: ${themeError.response?.data?.error?.message}`
      );
      console.log(
        `   Full Error: ${JSON.stringify(themeError.response?.data, null, 2)}`
      );
    }
    ...
    try {
      ...
      console.log(
        `   Error Code: ${invalidThemeError.response?.data?.error?.code}`
      );
      console.log(
        `   Error Message: ${invalidThemeError.response?.data?.error?.message}`
      );
    }
    ...
    try {
      ...
      console.log(
        `   Error Message: ${noTokenError.response?.data?.error?.message}`
      );
    }
