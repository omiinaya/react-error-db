  ...
  try {
    ...
    const editorElements = await page.evaluate(() => {
      const results = {
        ...
        hasToolbarButtons: document.querySelectorAll(
    ...
    try {
      ...
      await page.type('textarea', '**Bold text** and *italic text*', {
        delay: 100,
      });
    ...
    const errorMessages = await page.evaluate(() => {
      const errorElements = document.querySelectorAll(
        '.error, .text-red-500, .text-red-600, [style*="color: red"]'
      );
    ...
    await page.screenshot({
      path: 'editor-diagnostic-screenshot.png',
      fullPage: true,
    });
