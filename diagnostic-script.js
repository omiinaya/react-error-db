...
const diagnosticTests = {
  ...
  checkCSSLoading: () => {
    ...
    const cssCheck =
      document.querySelector('link[href*="react-markdown-editor-lite"]') ||
      document.querySelector(
        'style[data-vite-dev-id*="react-markdown-editor-lite"]'
      );
    ...
    if (cssCheck) {
      ...
      return {
        status: 'warning',
        message: 'CSS file not detected - may be loaded dynamically',
      };
    }
  },
  ...
  checkReactProps: () => {
    ...
    if (hasProps) {
      ...
      return {
        status: 'warning',
        message: 'React component props not detected (may need React DevTools)',
      };
    }
  },
  ...
  checkDependencies: () => {
    ...
    if (missing.length === 0) {
      ...
      return {
        status: 'fail',
        message: `Missing globals: ${missing.join(', ')}`,
      };
    }
  },
};
...
if (typeof window !== 'undefined') {
  console.log(
    '🎯 Markdown Editor Diagnostics loaded. Run `runMarkdownEditorDiagnostics()` to start tests.'
  );
}
