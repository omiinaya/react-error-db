...
test.describe('Home Page', () => {
  test('should display the home page with statistics', async ({ page }) => {
    ...
    // Check main heading
    await expect(
      page.getByRole('heading', { name: 'Welcome to Error Database' })
    ).toBeVisible();
    ...
    await expect(
      page.getByPlaceholder('Search error codes or messages...')
    ).toBeVisible();
    await expect(page.getByRole('button', { name: 'Search' })).toBeVisible();

    // Check navigation links
    await expect(
      page.getByRole('link', { name: 'Browse Errors' })
    ).toBeVisible();
    await expect(
      page.getByRole('link', { name: 'Contribute Solution' })
    ).toBeVisible();
  ...
  test('should navigate to search page', async ({ page }) => {
    ...
    await page
      .getByPlaceholder('Search error codes or messages...')
      .fill('test error');
    ...
    await expect(
      page.getByText('Search Results for "test error"')
    ).toBeVisible();
  });
  ...
  test('should navigate to errors page', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Error Database' })
    ).toBeVisible();
  });
});
