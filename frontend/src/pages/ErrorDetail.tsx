...
const ErrorDetail: React.FC = () => {
  ...
  const addSolutionMutation = useMutation({
    mutationFn: (data: CreateSolutionRequest) => api.addSolution(id!, data),
    onSuccess: () => {
      setSolutionText('');
  ...
  const deleteSolutionMutation = useMutation({
    mutationFn: (solutionId: string) => api.deleteSolution(solutionId),
    onSuccess: () => {
  ...
  return (
    <div className="container py-8 max-w-4xl mx-auto">
      ...
      {/* Common Causes */}
      {errorData.metadata?.commonCauses &&
        errorData.metadata.commonCauses.length > 0 && (
          <Card className="mb-8">
            <CardHeader>
            ...
            <CardContent>
              <ul className="list-disc list-inside space-y-1">
                {errorData.metadata.commonCauses.map(
          ...
          <div className="space-y-4">
            {solutions.map((solution: Solution) => (
              <Card
                key={solution.id}
                className={
                  solution.isVerified ? 'border-primary/20 bg-primary/5' : ''
                }
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <Avatar className="h-8 w-8">
                        ...
                        <AvatarFallback>
                          {solution.author.displayName?.[0]?.toUpperCase() ||
                            'U'}
                        </AvatarFallback>
                      </Avatar>
                      <div>
            ...
            <form onSubmit={handleSubmitSolution}>
              ...
              <Button
                ...
                disabled={
                  isSubmitting ||
                  !solutionText.trim() ||
                  solutionText.length > MAX_CHARACTERS
                }
                className={
                  solutionText.length > MAX_CHARACTERS
              ...
              >
                {isSubmitting
                  ? t('errors:detail.submitting')
                  : t('errors:detail.submitSolution')}
              </Button>
            </form>
          </CardContent>
        </Card>
