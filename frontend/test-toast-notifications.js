...
function testToastNotifications() {
      ...
      data: {
        ...
        error: {
          ...
          details: [
            {
              field: 'solutionText',
              message: 'Solution text must be at least 10 characters',
            },
          ],
        },
      },
  ...
  console.log(
    'Expected message: "solutionText: Solution text must be at least 10 characters"'
  );
  ...
  if (
    multiValidationError.response?.status === 400 &&
    multiValidationError.response?.data?.error?.code === 'VALIDATION_ERROR'
  ) {
...
console.log(
  '1. Show field-specific messages for validation errors (e.g., "solutionText: Solution text must be at least 10 characters")'
);
...
console.log(
  '5. Use longer duration (5s) for validation errors to give users more time to read'
);
console.log(
